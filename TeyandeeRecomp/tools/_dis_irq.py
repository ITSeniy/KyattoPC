"""Disassemble scene_irq / FC22 for scroll glitch analysis."""
...
def dis(data: bytes, start: int, nbytes: int = 0x100) -> None:
    i = 0
    pc = start
    ...
    while i < end:
        ...
        if size == 1:
            ...
            b1 = data[i + 1]
            extra = ""
            if name.startswith("B") and name in (
...
bank03 = (root / "banks" / "bank03.bin").read_bytes()
bank07 = (root / "banks" / "bank07.bin").read_bytes()

print("=== scene_irq A006 (bank03 high / 8k#7) ===")
# high half offset 0x2000 corresponds to model $A000 / CPU $8000 when R6 odd
dis(bank03[0x2006:], 0xA006, 0x140)

print("\n=== FC22 status-bar IRQ tail (bank07) ===")
dis(bank07[0x3C22:], 0xFC22, 0x90)

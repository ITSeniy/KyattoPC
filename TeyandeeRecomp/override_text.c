...
/* Map a CPU address inside a 16KB PRG bank image to a byte offset.
 * Switchable banks are analyzed/executed at $8000-$BFFF; the fixed bank
 * (last PRG bank on MMC1/MMC3) is at $C000-$FFFF. Both layouts share the
 * same 16KB bank buffer returned by runner_get_prg_bank_rw(). */
static int prg_addr_to_offset(uint16_t prg_addr) {
    if (prg_addr >= 0xC000) return (int)(prg_addr - 0xC000);
    if (prg_addr >= 0x8000) return (int)(prg_addr - 0x8000);
    return -1;
}
...
void text_override_patch_prg(int bank, uint16_t prg_addr,
    ...
    int offset = prg_addr_to_offset(prg_addr);
    if (offset < 0) {
        fprintf(stderr, "[TextOverride] patch_prg: addr $%04X out of $8000-$FFFF range\n",
                prg_addr);
        return;
    }

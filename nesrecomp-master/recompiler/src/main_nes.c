...
int main(int argc, char *argv[]) {
    ...
    {
        /* Pass 1: mark data_region / BRK-bogus rejects. */
        bool *reject = (bool *)calloc((size_t)funcs.count, sizeof(bool));
        if (reject) {
            for (int i = 0; i < funcs.count; i++) {
                const FunctionEntry *fe = &funcs.entries[i];
                if (fe->kind == FUNCTION_KIND_STANDALONE &&
                ...
                /* Drop entry-points in data_region (false code that mega-scans
                 * into real functions and truncates them at MAX_INSNS). */
                if (rom_addr_in_data_region(&cfg, fe->bank, fe->addr)) {
                    printf("[NESRecomp] Filtered data_region entry $%04X bank=%d\n",
                           fe->addr, fe->bank);
                    reject[i] = true;
                }
            }
            /* Pass 2: secondaries whose canonical was rejected become
             * standalone so they still get their own body (e.g. scene_init
             * $A093 was a secondary of false $8F7D). */
            for (int i = 0; i < funcs.count; i++) {
                FunctionEntry *fe = &funcs.entries[i];
                if (fe->kind != FUNCTION_KIND_SECONDARY) continue;
                if (rom_addr_in_data_region(&cfg, fe->bank, fe->addr)) continue;
                int ci = -1;
                for (int j = 0; j < funcs.count; j++) {
                    if (funcs.entries[j].addr == fe->canonical_addr &&
                        funcs.entries[j].bank == fe->canonical_bank) {
                        ci = j;
                        break;
                    }
                }
                if (ci >= 0 && reject[ci]) {
                    fe->kind = FUNCTION_KIND_STANDALONE;
                    fe->canonical_addr = fe->addr;
                    fe->canonical_bank = fe->bank;
                    printf("[NESRecomp] Promoted orphan secondary $%04X bank=%d to standalone\n",
                           fe->addr, fe->bank);
                }
            }
            /* Pass 3: compact. */

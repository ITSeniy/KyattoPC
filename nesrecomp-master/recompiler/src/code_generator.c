...
static int emit_instruction(FILE *f, const NESRom *rom, int bank,
    ...
    switch (e->mnemonic) {
        ...
        case MN_PHA:
            ...
            break;
        case MN_PLA:
            /* PLA PLA JMP abs — stack-unwind re-enter (e.g. Kyatto reverse-gravity
             * attach at $BC0C: discard JSR return and JMP $C00F into the game loop).
             * Without push_all_jsr the recomp model never pushed a return address, so
             * emitting two PLAs underflows S into $0100 and corrupts stack-page RAM.
             * Skip the PLAs and tail-dispatch the JMP target instead. With push_all_jsr
             * the PLAs are real and must run before the tail. */
            if (rom_read(rom, bank, (uint16_t)(pc + 1)) == 0x68 /* PLA */ &&
                rom_read(rom, bank, (uint16_t)(pc + 2)) == 0x4C /* JMP abs */) {
                uint16_t jtgt = (uint16_t)(
                    rom_read(rom, bank, (uint16_t)(pc + 3)) |
                    ((uint16_t)rom_read(rom, bank, (uint16_t)(pc + 4)) << 8));
                fprintf(f, "/* PLA PLA JMP $%04X — stack-unwind re-enter */\n", jtgt);
                if (cfg->push_all_jsr) {
            ...
            if (e->addr_mode == AM_ABS) {
                ...
                if (abs16 < 0x100) {
                    ...
                    if (sched_reset_addr) {
                        ...
                        /* 2-PHA indirect JSR: LDA #hi/PHA/LDA #lo/PHA […ops…] JMP (zp).
                         * Hardware: callee RTS returns to (hi:lo)+1.
                         * Recomp (no push_all_jsr): call_by_address is a C call, so
                         * drop the fake 6502 frame and goto the continuation.
                         * Scan back — table-build between PHA and JMP is common
                         * (e.g. Teyandee scene opcode dispatch at $A392–$A3AE). */
                        int found_2pha = 0;
                        uint16_t cont = 0;
                        for (int _back = 6; _back <= 48; _back++) {
                            if (pc < (uint16_t)(0x8000 + _back)) break;
                            uint16_t probe = (uint16_t)(pc - _back);
                            if (probe < func_base) break;
                            if (rom_read(rom, bank, probe)     == 0xA9 /* LDA # */ &&
                                rom_read(rom, bank, probe + 2) == 0x48 /* PHA */ &&
                                rom_read(rom, bank, probe + 3) == 0xA9 /* LDA # */ &&
                                rom_read(rom, bank, probe + 5) == 0x48 /* PHA */) {
                                uint8_t cont_hi = rom_read(rom, bank, probe + 1);
                                uint8_t cont_lo = rom_read(rom, bank, probe + 4);
                                cont = (((uint16_t)cont_hi << 8) | cont_lo) + 1;
                                /* ROM may push CPU window addr ($8xxx) while this
                                 * function is labeled in the 16KB high half ($Axxx). */
                                if (cont < func_base &&
                                    (uint16_t)(cont + 0x2000) >= func_base &&
                                    (uint16_t)(cont + 0x2000) < 0xC000) {
                                    cont = (uint16_t)(cont + 0x2000);
                                }
                                if (cont >= func_base && cont < 0xC000) {
                                    found_2pha = 1;
                                    break;
                                }
                            }
                    ...
                    for (int _back = 6; _back <= 48; _back++) {
                        ...
                        uint16_t probe = (uint16_t)(pc - _back);
                        if (probe < func_base) break;
                        if (rom_read(rom, bank, probe)     == 0xA9 &&
                            rom_read(rom, bank, probe + 2) == 0x48 &&
                            rom_read(rom, bank, probe + 3) == 0xA9 &&
                            rom_read(rom, bank, probe + 5) == 0x48) {
                            uint8_t cont_hi = rom_read(rom, bank, probe + 1);
                            uint8_t cont_lo = rom_read(rom, bank, probe + 4);
                            cont = (((uint16_t)cont_hi << 8) | cont_lo) + 1;
                            if (cont < func_base &&

...
/* 1 while the scanline renderer (or dot-PPU) is painting the visible frame and
 * servicing mid-frame IRQs. $2005 Y writes only update t mid-frame on real PPU
 * (not display v); the runner uses this flag so g_ppuscroll_y is not clobbered
 * by IRQ handlers that write Y=0 after $2006 (Teyandee scene_irq) or Y=$E0 for
 * status-bar t setup (FC22). Display Y mid-frame comes from $2006→v (hybrid
 * sync) or continues from the previous scanline's abs_nt_y. */
extern int     g_ppu_mid_frame_render;

/* Set when mid-frame $2006 reloads v (scene multi-split). Forces the scanline
 * renderer to re-base abs_nt_y even if g_ppuscroll_y's numeric value is
 * unchanged (common: both playfield and dialog use Y=0, but $2006 must jump
 * from "line 120 of continuous Y=0" back to nametable row 0). Without this,
 * one or more scanlines after a split fetch the wrong tile row (green seam /

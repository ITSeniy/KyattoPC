...
static void dot_fire_mmc3_irq(void) {
    ...
    /* Same mid-frame $2005 Y rule as ppu_render_frame (t-only). */
    g_ppu_mid_frame_render = 1;
    func_IRQ();
    g_ppu_mid_frame_render = 0;
    /* Hybrid: Y + NT Y from v after $2006; NT X / rest from $2000; X from $2005. */
...
static void dot_render_scanline(int sy, uint32_t *target, int snapshot) {
    ...
    if (!(g_ppumask & 0x08)) {
         ...
         * or any Y-scroll-source change (mid-frame split), including $2006
         * reloads that leave the scroll_y byte numerically unchanged. */
        if (!s_v_init || g_ppuscroll_y != s_last_sy_y || cur_nt_yb != s_last_nt_yb
            || g_ppu_force_y_reload) {
...
/* Paint one visible scanline, then clock its MMC3 A12 edge. Hardware raises
 * A12 near the END of the scanline (sprite fetch), so the IRQ handler's
 * scroll/CHR writes apply to subsequent lines — matching ppu_render_frame. */

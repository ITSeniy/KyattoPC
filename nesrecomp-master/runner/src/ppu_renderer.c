...
void ppu_render_frame(uint32_t *framebuf) {
...

    {
        ...
        /* MMC3 A12 scanline counter: hardware clocks once on the pre-render
         * line (-1) and once near the END of each visible line (dots ~260–320
         * when A12 rises during sprite fetches if BG is at $0000). The IRQ
         * handler therefore runs after the triggering line has been painted;
         * scroll/CHR writes take effect on the NEXT scanline.
         *
         * Previously we serviced the IRQ BEFORE drawing sy, which applied the
         * new scroll one line early — a full-width wrong tile-row at every
        ...
        for (int sy = 0; sy < 240; sy++) {
             ...
             * g_ppu_force_y_reload: mid-frame $2006 absolute Y load (even when
             * the extracted scroll_y byte matches the previous value). */
            int cur_nt_y_bit = (ppuctrl_row & 0x02) ? 1 : 0;
            int force_y = g_ppu_force_y_reload;
            if (!v_initialized
                ...
                /* Reset abs_nt_y when Y source changes OR $2006 reloaded v.
                 * Sprite-0 HUD/game with the same scroll_y and no $2006 must
                 * NOT reset — the incremental counter stays correct. */
                if (abs_nt_y < 0
                    || scroll_y != v_last_scroll_y
                    || cur_nt_y_bit != v_last_ppuctrl_nt
                    || force_y) {

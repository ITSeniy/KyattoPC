...
int     g_ppu_mid_frame_render = 0;   /* 1 during visible scanline paint + MMC3 IRQ */
int     g_ppu_force_y_reload   = 0;   /* 1 after mid-frame $2006 → force abs_nt_y rebase */
    ...
    switch (reg) {
        ...
        case 0x2005: {
            ...
            if (!g_ppuaddr_latch) {
                 ...
                 * Fine X applies immediately on real PPU; coarse X lands via
                 * the per-scanline t→v horizontal copy. Our scanline renderer
                 * uses g_ppuscroll_x as the full X for subsequent lines. */
                s_ppu_t = (s_ppu_t & 0xFFE0) | ((uint16_t)val >> 3);
                s_ppu_fine_x = val & 7;
                g_ppuscroll_x = val;
            } else {
                /* Second write (w=1): fine Y → t[12:14], coarse Y → t[5:9].
                 * On real PPU this updates t ONLY — v (display Y) is unchanged
                 * mid-frame. Games like Teyandee scene_irq always STA #$00 to
                 * $2005 after an optional $2006 pair; applying that to
                 * g_ppuscroll_y wiped the Y set by $2006 on X-only splits
                 * (shake/parallax when $0463==0), producing a one-scanline
                 * seam of wrong nametable rows (and right-edge tear when
                 * combined with $2000 NT bits). Status-bar FC22 writes Y=$E0
                 * the same way for next-frame t; display Y must continue via
                 * abs_nt_y. NMI/VBlank writes (mid_frame==0) still set
                 * g_ppuscroll_y for the upcoming frame. */
                s_ppu_t = (s_ppu_t & 0x0C1F) |
                          ((uint16_t)(val & 7) << 12) |
                          ((uint16_t)(val >> 3) << 5);
                if (!g_ppu_mid_frame_render)
                    g_ppuscroll_y = val;
